...
"""Import idempotent des guides chiffrage / Calcul Devis depuis docs/."""
...
def execute():
	...
	max_ordre = frappe.db.sql(
		"SELECT COALESCE(MAX(ordre_affichage), 0) FROM `tabGuide Utilisation`"
	)[0][0]
	ordre = int(max_ordre) + 10

	for entry in GUIDES:
		if entry["id"] not in NEW_GUIDE_IDS:
			continue
